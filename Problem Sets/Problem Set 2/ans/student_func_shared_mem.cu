...
__global__ void gaussian_blur(const unsigned char* const inputChannel,
  ...
  // Store convolution kernel in shared mem for more fewer memory transaction
  const int tid = threadIdx.y * blockDim.x + threadIdx.x;
  extern __shared__ float conv_kernel[];
  conv_kernel[tid] = filter[tid];
  __syncthreads();

  const int2 pixel_pos = make_int2(blockIdx.x * blockDim.x + threadIdx.x,
                                   blockIdx.y * blockDim.y + threadIdx.y);
  if (pixel_pos.x >= numCols || pixel_pos.y >= numRows) {
    return;
  }

  // Convolve the input with mask
  float w_sum = 0;
  const int pixel_pos_abs = pixel_pos.y * numCols + pixel_pos.x;

  for (int i = -filterWidth / 2; i <= filterWidth / 2; ++i) {
    auto neighbour_y = min(max(pixel_pos.y + i, 0), numRows - 1);
    for (int j = -filterWidth / 2; j <= filterWidth / 2; ++j) {
      auto neighbour_x = min(max(pixel_pos.x + j, 0), numCols - 1);
      auto mask_idx =
          (i + filterWidth / 2) * filterWidth + (j + filterWidth / 2);
      auto curr_pixel = neighbour_y * numCols + neighbour_x;
      w_sum += conv_kernel[mask_idx] * inputChannel[curr_pixel];
    }
  }
  outputChannel[pixel_pos_abs] = w_sum;
}
...
__global__ void separateChannels(const uchar4* const inputImageRGBA,
  ...
  redChannel[pixel_pos_abs] = inputImageRGBA[pixel_pos_abs].x;
  greenChannel[pixel_pos_abs] = inputImageRGBA[pixel_pos_abs].y;
  blueChannel[pixel_pos_abs] = inputImageRGBA[pixel_pos_abs].z;
}
...
void your_gaussian_blur(const uchar4* const h_inputImageRGBA,
  ...
  // channel.
  const size_t shared_mem_size = sizeof(float) * filterWidth * filterWidth;

  gaussian_blur<<<gridSize, blockSize, shared_mem_size>>>(
